...
class BuildField(Builder):
    ...
    def set_parser_location_to_build(self):
        ...
        if field_link:
            ...
            if self.is_enough_crop():
                ...
                #zmieniona nazwa na polskie crop -> zboże
                field_link = self.link_to_field_to_build('zboże')
                full_field_link = SERVER_URL + field_link
                field_page = self.session.get(full_field_link).text
                self.parser_location_to_build = BeautifulSoup(field_page, 'html.parser')
        else:
    ...
    def minimal_resource(self):
        ...
        #Nie potrzebne ponieważ używane są polskie nazwy
    ...
    def link_to_field_to_build(self, minimal_resource):
        ...
        for field, link in fields.items():
            ...
            if((minimal_resource == 'drewno') and ('las' in field.lower()) and (fields_level < lowest_level)):
                lowest_level = fields_level
                field_link = link
            elif((minimal_resource == 'glina') and ('gliny' in field.lower()) and (fields_level < lowest_level)):
                lowest_level = fields_level
                field_link = link
            elif((minimal_resource == 'żelazo') and ('żelaza' in field.lower()) and (fields_level < lowest_level)):
                lowest_level = fields_level
                field_link = link
            elif((minimal_resource == 'zboże') and ('pole' in field.lower()) and (fields_level < lowest_level)):
                lowest_level = fields_level
                field_link = link
            """
