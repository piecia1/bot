...
class Builder(ABC):
    ...
    def parse_required_resources(self):
        #Tutaj wprowadzone zmainy int(span.contents[1].contents[0])
    ...
    def parse_resources_amount(self):
        ...
        resources_amount = {'drewno': lumber, 'glina': clay,
                            'żelazo': iron, 'zboże': crop}
    ...
    def is_enough_crop(self):
        # W tej funkcji potrzebne sa zmiany - polskie nazwy
        ...
        if parse_status_messages:
            ...
            if parse_message == 'Za niska produkcja żywności: rozbuduj najpierw pola zboża':
                return False
    ...
    def set_parser_location_to_build(self):
        ...
        #build_page_html = self.session.get('https://ts3.travian.pl/build.php?id=4').text
        #self.parser_location_to_build = BeautifulSoup(build_page_html, 'html.parser')
